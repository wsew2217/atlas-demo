...
const FACTORY_QUERY = 'Plant B'
const FACTORY_NAME = 'Plant B · Cambodia'
const USER_NAME = 'Sothea Heng'

export default async function FactoryBDashboard() {
    ...
    <>
      <header className="border-b border-[var(--border)] bg-[var(--cream)]/80 backdrop-blur">
        <div className="mx-auto flex w-full max-w-7xl items-center justify-between px-6 py-4">
          <Link href="/demo/factory/plant-b" className="flex items-baseline gap-3">
        ...
        <div className="space-y-6">
          {batches.length === 0 ? (
            <p className="rounded-lg border border-dashed border-[var(--border)] bg-[var(--surface)] p-12 text-center text-[var(--muted)]">
              No batches assigned to {FACTORY_NAME} right now.
            </p>
          ) : (
            batches.map((batch) => {
