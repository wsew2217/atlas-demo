...
export default async function FullDemoCustomersPage() {
  const [brands, orders] = await Promise.all([listBrands(), listOrders()])
      ...
      <header className="mb-8 flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm text-[var(--muted)]">
            {brands.length} active customers · {orders.length} POs across all customers
          </p>
        </div>
        <span
          ...
          title="Onboarding workflow coming next round"
        >
          + Onboard new customer (coming)
        </span>
      </header>
